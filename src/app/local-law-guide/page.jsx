...
import { fetchPosts, fetchTotalPosts, fetchCategories, transformPost } from "@/lib/wordpress";
...
const LocalLawGuidePage = () => {
  ...
  /**
   * Fetch all posts from WordPress API, filtered by local-law-guide category
   */
  const fetchLocalLawPosts = async () => {
    try {
      setLoading(true);
      setError(null);

      // First, find the local-law-guide category ID
      const allCategories = await fetch(`${WORDPRESS_CONFIG.API_URL}/categories?per_page=50`);
      
      if (!allCategories.ok) {
        console.error(`Local law categories API returned ${allCategories.status}: ${allCategories.statusText}`);
      ...
      if (!Array.isArray(categories)) {
        ...
        setError('Invalid categories response. Please try again later.');
        setLoading(false);
        return;
      }
      
      // Find the local-law-guide category by slug or name
      const localLawCategory = categories.find(cat => 
        cat.slug === 'local-law-guide' || 
        cat.name.toLowerCase() === 'local law guide' ||
        cat.name.toLowerCase().includes('local law guide')
      );
      ...
      // Use the main fetchPosts function to get properly transformed posts
      const transformedPosts = await fetchPosts(1, 100, localLawCategory.id);
      
      setPosts(transformedPosts);
      
    } catch (err) {
      console.error('Error fetching local law posts:', err);
      setError('Failed to load local law guides. Please try again later.');
    } finally {
      setLoading(false);
    }
  };

  // Using the imported transformPost function from wordpress.js
          ...
          <div className="my-8 md:mb-12">
            ...
            <div className="mb-6 lg:mb-8">
              ...
              <p className="text-[#89A096] text-lg md:text-xl mt-2 font-poppins">
                Comprehensive guides and resources for NYC local law compliance, including Local Law 11, Local Law 152, and building code requirements
              </p>
            </div>
            ...
            <div className="w-full">
              <div className="relative w-full">
                <div className="relative">
                  ...
                  <input
                    type="text"
                    placeholder="Search local law guides..."
                    value={searchQuery}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 lg:gap-8">
            ...
            {loading && (
              <div className="col-span-full text-center py-12">
                ...
                <p className="text-[#89A096] mt-4">Loading local law guides...</p>
              </div>
            )}
            ...
            {!searchQuery && !loading && !error && displayPosts.length === 0 && (
              <div className="col-span-full text-center py-12">
                <p className="text-[#89A096] text-lg">No local law guides found in this category.</p>
              </div>
            )}
...
export default LocalLawGuidePage;
